...
def _pathspec(excluded_prefixes: list[str]) -> list[str]:
    out=["."]
    for raw in excluded_prefixes:
        rel=raw.replace("\\","/").strip("/")
        if not rel or rel in {".",".."} or rel.startswith("../"):
            raise ValueError(f"unsafe excluded launcher path: {raw}")
        out.extend([f":(exclude){rel}",f":(exclude){rel}/**"])
    return out


def tracked_changes(root: Path, baseline_ref: str, excluded_prefixes: list[str] | None = None) -> tuple[list[str],list[str],list[str],list[str]]:
    pathspec=_pathspec(excluded_prefixes or [])
    raw=run(["git","diff","--name-status","-z",baseline_ref,"--",*pathspec],root).stdout
    ...
    untracked=run(["git","ls-files","-z","--others","--exclude-standard","--",*pathspec],root).stdout
...
def capture(root: Path, baseline_ref: str, excluded_prefixes: list[str] | None = None) -> dict[str,Any]:
    oid=resolve_ref(root,baseline_ref); excluded=list(dict.fromkeys(excluded_prefixes or []))
    _pathspec(excluded)
    return {"format":"dsd-scope-baseline-v2.1","project_root":str(root),"baseline_ref":baseline_ref,"baseline_oid":oid,"excluded_prefixes":excluded,"captured_at":now()}


def compare(root: Path, baseline: dict[str,Any]) -> dict[str,Any]:
    ref=str(baseline.get("baseline_ref") or ""); ensure_ref(root,ref)
    excluded=[str(x) for x in baseline.get("excluded_prefixes",[]) if str(x)] if isinstance(baseline.get("excluded_prefixes"),list) else []
    changed,added,removed,modified=tracked_changes(root,ref,excluded)
    return {
        ...
        "excluded_prefixes":excluded,"changed_since_attempt_baseline":changed,"changed_count":len(changed),
...
def parser()->argparse.ArgumentParser:
    ...
    p=sub.add_parser("capture"); p.add_argument("--root",type=Path,required=True); p.add_argument("--baseline-ref",required=True); p.add_argument("--output",type=Path,required=True); p.add_argument("--exclude-prefix",action="append",default=[])
...
def main()->int:
    ...
    try:
        ...
        if args.command=="capture":
            data=capture(root,args.baseline_ref,args.exclude_prefix); write_new(args.output.resolve(),data); print(f"Captured attempt checkpoint {args.baseline_ref} -> {args.output.resolve()}"); return 0
