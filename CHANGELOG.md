...
- Replaced the coarse supersession padlock with delta-aware retention. Read-only/result rooms, empty mutable rooms, frozen `carry_from`, explicit Analyst `rederive_from_primary`, and deltas already materialized in primary can release immediately; only unique undisposed mutable deltas remain protected. Forced discard requires an auditable reason and cannot bypass that protection.
- Made launcher fixtures invisible to worker-output evidence by construction. Fixture prefixes are frozen into attempt scope exclusions and omitted from checkpoint/carry/integration diffs; fixture/write-authority overlap is rejected mechanically, preventing launcher-provisioned `node_modules` or similar inputs from leaking into accepted patches.
- Fixed stale-room detection for already-dirty tracked files by comparing frozen snapshot bytes against the current primary working tree, not just HEAD plus porcelain status. Clean cold rooms refresh; rooms with task delta remain protected.
- Added explicit generated-output derivations so an out-of-allowlist generated counterpart may pass only when its declared in-scope source changed in the same attempt; generated-only movement stays red. Wired the missing `verification-result` CLI transition and accepts one exact opening `Disposition: TOKEN` compatibility header without classifying prose.
- Added regression/E2E/behavioral coverage for automatic retirement, read-only supersession, fixture exclusion, same-status stale bytes, generated-output provenance, explicit verification disposition, and completed-run purge.
