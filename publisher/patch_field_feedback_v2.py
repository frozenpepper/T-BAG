...
def replace_n(path: str, old: str, new: str, expected: int) -> None:
    ...
    count=text.count(old)
    if count != expected:
        raise SystemExit(f'{path}: expected {expected} targets, found {count}: {old[:140]!r}')
    p.write_text(text.replace(old,new),encoding='utf-8')


# --- Routing-token normalization: tolerate syntax, never infer semantics. ---
...
# --- Advance: quarantine an isolated deterministic error for this pass. ---
...
# --- Parent tick: do not hand quarantined actions back as executable work. ---
...
# --- OpenCode credential rotation: recovery stays lifecycle-owned. ---
...
# --- Tests: SQR8V2 observed syntax + positive controls + poison continuation. ---
