...
class ComponentsTests(unittest.TestCase):
    ...
    def test_opencode_plugin_uses_stable_follow_tool_and_session_wake_not_background_subagent_protocol(self):
        ...
        self.assertIn('../tbag-opencode-transport-core.js',text)
        self.assertIn('tbag_follow: tool({',text); self.assertNotIn('tbag_launch: tool({',text)
        self.assertIn('"tool.execute.after"',text); self.assertIn('launchResultsFromToolOutput',text); self.assertIn('repairObserversFromTick',text)
        ...
        self.assertIn('"tool.execute.before"',text); self.assertIn('must not foreground dsd_attempt.py follow',text)
        ...
        self.assertIn('runHeartbeats',text); self.assertIn('startHeartbeatTimers',text); self.assertIn('pulseRun',text)
        self.assertIn('COMPLETION_PULSE_MS',core); self.assertIn('HEALTH_HEARTBEAT_MS',core); self.assertIn('completionTimer.unref',core); self.assertIn('healthTimer.unref',core)
