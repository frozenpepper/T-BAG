...
class DocsTests(unittest.TestCase):
    ...
    def test_owner_reporting_contract_is_short_and_self_contained(self):
        ...
        self.assertIn("sell task counts as progress",hot)
        self.assertIn("native question UI",hot)
        self.assertIn("━━ T-BAG UPDATE ━━",hot)
    ...
    def test_parent_harness_wake_contract_is_canonical_in_opencode(self):
        ...
        self.assertIn("native interactive question surface",harness)
        self.assertIn("degrade conversation-first",harness)
        self.assertIn("`OPENCODE.md` is the sole protocol",hot)
        self.assertIn("60s deterministic completion pulse",hot)
        self.assertIn("Never run core `follow`",hot)
        self.assertIn("normal autonomy does not depend on that custom tool",oc)
        self.assertIn("native `question` tool",oc)
        self.assertIn("parent_tick.py tick",oc); self.assertIn("diagnostics only",oc)
        self.assertIn("Canonical OpenCode loop",oc)
        self.assertIn("live generation proof",oc)
        self.assertIn("bootstrap_ready=false",oc)
        self.assertIn("60-second deterministic completion pulse",oc)
        self.assertIn("per-attempt wakes are the primary completion edge",oc.lower())
        self.assertIn("in-memory wake bit",oc)
        self.assertIn("never polls idleness",oc)
        self.assertIn("observer repair is tick-driven",oc)
    ...
    def test_long_running_attempts_remain_inspectable(self):
        ...
        self.assertIn("Tick monitoring distinguishes active work",hot)
        self.assertIn("slower health heartbeat",hot)
        self.assertIn("parent_tick.py tick",prompts)
        self.assertIn("final-report/no-terminal",prompts)
        self.assertIn("silent anomalies",prompts)
        self.assertIn("dsd_attempt.py inspect",prompts)
        self.assertIn("tbag_follow",prompts)
