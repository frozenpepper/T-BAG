...
The installer detects the **active parent generation** first. OpenCode 1.x and 2.x both export `OPENCODE_PID`; T-BAG inspects only that exact host process, then version-probes the matching executable. This resolves dual installs where PATH `opencode` is 1.x while the running parent is `opencode2`. Outside an OpenCode-owned child process, the historical `opencode --version` probe remains primary and `opencode2` is tried only when `opencode` is absent. OpenCode 2 beta strings such as `opencode2 v0.0.0-beta-*` are generation 2 despite their package version beginning with zero. The installer then installs the matching **server transport and presentation companion**. OpenCode 1.x receives the legacy hook transport plus the v1 `@opencode-ai/plugin/tui` companion and TUI-config entry; OpenCode 2.x receives the native `id` + `setup(ctx)` transport plus the separate `tbag-ui/` companion. T-BAG does not add host plugin packages to the project; the host owns those runtime dependencies.
