...
- structured tick/launch stdout is an acceleration surface, not transport authority: keep it unfiltered for immediate auto-arm, while the deterministic durable pulse can restore heartbeat state and missing observers if shell presentation mangles that output;
