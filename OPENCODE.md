...
For CI/offline/headless maintenance, pass `--headless` (or `TBAG_HEADLESS=1`; `CI` is also recognized). T-BAG deliberately does **not** infer headlessness from TTY state because a real OpenCode tool shell may itself be non-TTY. Headless installation reports `degraded_manual=true`, emits no unfulfillable restart question, and requires explicit parent ticks until a live host proves the token. Re-running an unchanged installer also preserves the existing activation request instead of rewriting its timestamp. The installer is bootstrap/update machinery, not a runtime-health probe.
